...
enum State {
    Idle,
    IssueReadData,
    ReadDataResponseHeader {
        index: u8,
    },
    ReadDataResponseLength,
    ReadDataResponseBody {
        remaining: u16,
        in_buffer_len: u16,
        in_buffer_offset: u16,
    },
    IssueWriteData,
}
...
impl<'queue, Spi, CsPin, IrqPin, SyncPin>
    WioTerminalSPITransport<'queue, Spi, CsPin, IrqPin, SyncPin>
where
    Spi: spi::Transfer<u8> + spi::Write<u8>,
    CsPin: OutputPin,
    IrqPin: InputPin,
        ...
        loop {
            match self.state {
                ...
                State::ReadDataResponseLength => {
                    ...
                    self.state = State::ReadDataResponseBody {
                        remaining: length,
                        in_buffer_len: 0,
                        in_buffer_offset: 0,
                    };
                }
                State::ReadDataResponseBody {
                    mut remaining,
                    mut in_buffer_len,
                    mut in_buffer_offset,
                } => {
                    if in_buffer_len == 0 {
                        ...
                        let _ = ScopedGuard::new(|| self.deassert_cs());
                        let bytes_to_read =
                            core::cmp::min(self.buffer.len(), remaining as usize) as u16;
                        let result = self
                            .spi
                            .transfer(&mut self.buffer[..bytes_to_read as usize]);
                        if result.is_err() {
                            self.state = State::Idle;
                            return;
                        }
                    ...
                    while in_buffer_offset < in_buffer_len {
                        if let Err(_e) = self.rq.enqueue(self.buffer[in_buffer_offset as usize]) {
                            break;
                        }
                        in_buffer_offset += 1;
                    }
