...
use wio_terminal::hal::{
    clock::GenericClockController, time::KiloHertz, time::Miliseconds, time::Nanoseconds,
};
use wio_terminal::{gpio, hal, pac, sercom, spi_master_lcd, spi_master_rtl8720, Pins};
...
use atat::{atat_derive::*, ClientBuilder, ComQueue, NoopUrcMatcher, Queues, ResQueue, UrcQueue};
...
type LcdSpi = SpiInterface<
    SpiLcd,
    gpio::Pb21<gpio::Output<gpio::PushPull>>,
    gpio::Pc6<gpio::Output<gpio::PushPull>>,
>;
type Lcd = Ili9341<LcdSpi, gpio::Pc7<gpio::Output<gpio::PushPull>>>;
...
const APP: () = {
    ...
    fn init(context: init::Context) -> init::LateResources {
        ...
        let rtl_transport = WioTerminalSPITransport::new(
            spi_rtl,
            rtl_cs,
            rtl_irq0,
            rtl_sync,
        ...
        let _rtl_config: atat::Config = atat::Config::default();
        ...
        let _rx = esp_at::WioTerminalSPITransportRx::new(read_consumer);
    ...
    fn ui_loop(context: ui_loop::Context) {
        ...
        *ui_interval = if *ui_interval < 33u32 {
            *ui_interval + 1u32
        } else {
            0
        };
        ...
        *led_interval = if *led_interval < 500u32 {
            *led_interval + 1u32
        } else {
            0
        };
